...
\begin{tikzpicture}[%every node/.style={color=white}
  ...
  \draw [ultra thick] (nic) -- ($(nic) - (0,1)$);

  \node [draw,thick,dotted,rectangle, minimum width=8cm, minimum height=2cm] at($(host.south) - (1.5,1.5)$) (infrastructure) {};
  \node [anchor=north] at($(infrastructure.south)$) {\bfseries{}Institute IT Infrastructure};

  \node [draw,thick,rectangle, minimum width=2.6cm, minimum height=1.8cm, text width=2.5cm,align=center] at($(infrastructure)!.5!(infrastructure.west)$) (hpc) {\textbf{HPC}\\($2 PB$ storage)};

  \node [draw,thick,rectangle, minimum width=2.6cm, minimum height=1.8cm, text width=2.5cm, align=center] at($(infrastructure)!.5!(infrastructure.east)$) (fileserver) {\textbf{Central\\Storage}\\($2.5 PB$ storage)};

  \foreach \n in {1,...,4}
  {
   
    \draw [ultra thick] ($(hpc.north east)!\n*.2!(hpc.south east)$) -- ($(fileserver.north west)!\n*.2!(fileserver.south west)$);
  }
